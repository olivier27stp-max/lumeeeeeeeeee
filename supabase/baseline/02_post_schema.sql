...
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types) values ('support-captures', 'support-captures', false, 8388608, array['image/*']::text[]) on conflict (id) do nothing;
...
select cron.schedule('lume_retention_logs', '20 4 * * *', $c$select public.run_retention_logs()$c$);
select cron.schedule('lume_sync_auth_telemetry', '*/15 * * * *', $c$select public.sync_auth_telemetry()$c$);
select cron.schedule('lumi_expire_reservations', '*/5 * * * *', $c$select public.expire_ai_reservations(5)$c$);
