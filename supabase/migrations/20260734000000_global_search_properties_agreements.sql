-- ════════════════════════════════════════════════════════════════════
-- Global Search V4 — Properties + Agreements as searchable entities
--
-- 1) search_global_source gains two new entity types:
--    - 'property'  → public.properties (name, address, city, postal code,
--                    owner client name) — clicking lands on the client hub
--                    properties section.
--    - 'agreement' → public.job_agreements joined to jobs (job title/number,
--                    client name, property address, signer name) — clicking
--                    lands on the job hub.
-- 2) New column extra_ref_id on all search functions:
--    - property  → client_id (navigate to /clients/:clientId?property=:id)
--    - agreement → job_id    (navigate to /jobs/:jobId)
--    - others    → null (events carry their job_id for future use)
--    Return types change, so the four functions are dropped and recreated.
-- 3) Trigram indexes for property/agreement search performance.
--
-- Idempotent — safe to run multiple times.
-- Apply with: npx tsx scripts/apply-sql.ts supabase/migrations/20260734000000_global_search_properties_agreements.sql
-- ════════════════════════════════════════════════════════════════════
...
create index if not exists properties_search_trgm
  ...
  using gin (lower(concat_ws(' ', name, address, street_number, street_name, city, province, postal_code)) gin_trgm_ops);

create index if not exists job_agreements_signer_trgm
  on public.job_agreements
  using gin (lower(coalesce(signer_name, '')) gin_trgm_ops);

-- ── 2. Drop old functions (return type changes) ─────────────────────
...
-- ── 3. search_global_source with properties + agreements ────────────
...
-- ── 4. Recreate dependent functions with extra_ref_id ────────────────
