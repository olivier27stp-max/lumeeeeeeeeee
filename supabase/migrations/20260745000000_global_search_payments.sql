-- ════════════════════════════════════════════════════════════════════
-- Global Search V5 — Payments as a searchable entity
--
-- 1) search_global_source gains 'payment' (public.payments):
--    matched by client name/email, linked invoice number, method/provider,
--    and by AMOUNT — "1092", "1,092.26", "1092,26" or "$1092.26" all match
--    a $1,092.26 payment (whole-dollar queries match any cents).
--    extra_amount_cents carries the amount; extra_ref_id = invoice_id so
--    the UI can open the invoice hub.
-- 2) Same 13-column signature as V4 (20260734000000). Functions are
--    dropped/recreated so this applies cleanly whether or not V4 ran —
--    but apply V4 first anyway: it creates the properties/agreements
--    trigram indexes this version still relies on.
--
-- Idempotent — safe to run multiple times.
-- Apply with: npx tsx scripts/apply-sql.ts supabase/migrations/20260745000000_global_search_payments.sql
-- ════════════════════════════════════════════════════════════════════

begin;

-- ── 1. Index for amount lookups ──────────────────────────────────────
create index if not exists payments_org_amount_idx
  on public.payments (org_id, amount_cents);

-- ── 2. Drop old functions (recreate below) ───────────────────────────
drop function if exists public.search_global(uuid, text, int, int);
drop function if exists public.search_global_by_type(uuid, text, text, int, int);
drop function if exists public.search_global_counts(uuid, text);
drop function if exists public.search_global_source(uuid, text);

-- ── 3. search_global_source with payments ────────────────────────────
create function public.search_global_source(p_org uuid, p_q text)
returns table (
  ...
  extra_client_id uuid, extra_client_name text, extra_ref_id uuid,
  created_at timestamptz, rank double precision
)
...
as $$
  ...
  query_terms as (
    select a.q, a.q_digits, ('%' || a.q || '%')::text as pattern,
      ...
      -- Amount query → cents. Accepts "1092", "1,092.26", "1092,26", "$1 092.26".
      case when a.raw_q ~ '^[\s$]*[0-9][0-9.,\s$]*$' then
        (case
           when regexp_replace(a.raw_q, '[^0-9.,]', '', 'g') ~ '^[0-9]{1,3}(,[0-9]{3})+(\.[0-9]{1,2})?$'
             then round(replace(regexp_replace(a.raw_q, '[^0-9.,]', '', 'g'), ',', '')::numeric * 100)::integer
           when regexp_replace(a.raw_q, '[^0-9.,]', '', 'g') ~ '^[0-9]{1,10}(\.[0-9]{1,2})?$'
             then round(regexp_replace(a.raw_q, '[^0-9.,]', '', 'g')::numeric * 100)::integer
           when regexp_replace(a.raw_q, '[^0-9.,]', '', 'g') ~ '^[0-9]{1,10},[0-9]{1,2}$'
             then round(replace(regexp_replace(a.raw_q, '[^0-9.,]', '', 'g'), ',', '.')::numeric * 100)::integer
           else null
         end)
      else null end as q_amount_cents,
      -- Whole-dollar query (no cents typed) → also match any cents value
      (a.raw_q ~ '^[\s$]*[0-9][0-9,\s$]*$') as q_amount_whole
  ...
  clients_ranked as (
    select 'client'::text as entity_type, c.id as entity_id,
      ...
      null::uuid as extra_ref_id, c.created_at,
  ...
  properties_ranked as (
    select 'property'::text as entity_type, p.id as entity_id,
      coalesce(nullif(p.name, ''), nullif(p.address, ''), 'Property') as title,
      coalesce(
        nullif(p.address, ''),
      ...
      case when p.is_primary then 'primary' else null end as extra_status,
      null::integer as extra_amount_cents, null::text as extra_currency,
      null::text as extra_date,
      p.client_id as extra_client_id,
      coalesce(nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(c.company, '')) as extra_client_name,
      p.client_id as extra_ref_id, p.created_at,
      (ts_rank_cd(to_tsvector('simple', lower(concat_ws(' ', p.name, p.address, p.street_number, p.street_name, p.city, p.province, p.postal_code, c.first_name, c.last_name, c.company))), qt.tsq) * 2.0
        + greatest(
            similarity(lower(coalesce(p.name, '')), qt.q),
            similarity(lower(coalesce(p.address, '')), qt.q),
            similarity(lower(concat_ws(' ', p.city, p.province, p.postal_code)), qt.q),
            similarity(lower(concat_ws(' ', c.first_name, c.last_name, c.company)), qt.q)
          ))::double precision as rank
    from public.properties p
    left join public.clients c on c.id = p.client_id and c.org_id = p_org and c.deleted_at is null
    join query_terms qt on true
    where p.org_id = p_org and p.deleted_at is null
      and (
        lower(concat_ws(' ', p.name, p.address, p.street_number, p.street_name, p.city, p.province, p.postal_code)) ilike qt.pattern
        or similarity(lower(concat_ws(' ', p.name, p.address, p.city)), qt.q) > 0.12
        or to_tsvector('simple', lower(concat_ws(' ', p.name, p.address, p.street_number, p.street_name, p.city, p.province, p.postal_code))) @@ qt.tsq
      )
  ),
  jobs_ranked as (
    select 'job'::text as entity_type, j.id as entity_id,
      ...
      j.scheduled_at::text as extra_date, j.client_id as extra_client_id, j.client_name as extra_client_name,
      null::uuid as extra_ref_id, j.created_at,
  ...
  agreements_ranked as (
    select 'agreement'::text as entity_type, a.id as entity_id,
      coalesce(nullif(j.title, ''), nullif(j.job_number, ''), 'Agreement') as title,
      coalesce(nullif(a.signer_name, ''), nullif(j.client_name, ''), nullif(j.property_address, ''), 'Agreement') as subtitle,
      a.status as extra_status,
      null::integer as extra_amount_cents, null::text as extra_currency,
      coalesce(a.signed_at, a.sent_at, a.created_at)::text as extra_date,
      coalesce(a.client_id, j.client_id) as extra_client_id,
      coalesce(nullif(j.client_name, ''), nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(c.company, '')) as extra_client_name,
      a.job_id as extra_ref_id, a.created_at,
      (ts_rank_cd(to_tsvector('simple', lower(concat_ws(' ', j.title, j.job_number, j.client_name, j.property_address, a.signer_name, c.first_name, c.last_name, c.company, c.email))), qt.tsq) * 2.0
        + greatest(
            ...
            similarity(lower(coalesce(a.signer_name, '')), qt.q)
          ))::double precision as rank
    from public.job_agreements a
    join public.jobs j on j.id = a.job_id and j.org_id = p_org and j.deleted_at is null
    left join public.clients c on c.id = coalesce(a.client_id, j.client_id) and c.org_id = p_org
    join query_terms qt on true
    where a.org_id = p_org and a.deleted_at is null
      and (
        lower(concat_ws(' ', j.title, j.job_number, j.client_name, j.property_address, a.signer_name)) ilike qt.pattern
        or similarity(lower(concat_ws(' ', j.title, j.job_number, j.client_name, a.signer_name)), qt.q) > 0.12
        or to_tsvector('simple', lower(concat_ws(' ', j.title, j.job_number, j.client_name, j.property_address, a.signer_name))) @@ qt.tsq
      )
  ),
  payments_ranked as (
    select 'payment'::text as entity_type, pay.id as entity_id,
      'Payment'::text as title,
      coalesce(nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(c.company, ''), nullif(i.invoice_number, ''), nullif(pay.method, ''), pay.provider, 'Payment') as subtitle,
      pay.status as extra_status,
      pay.amount_cents as extra_amount_cents,
      pay.currency as extra_currency,
      pay.payment_date::text as extra_date,
      pay.client_id as extra_client_id,
      coalesce(nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(c.company, '')) as extra_client_name,
      pay.invoice_id as extra_ref_id, pay.created_at,
      (greatest(
          similarity(lower(concat_ws(' ', c.first_name, c.last_name, c.company)), qt.q),
          similarity(lower(coalesce(i.invoice_number, '')), qt.q),
          case when qt.q_amount_cents is not null and pay.amount_cents = qt.q_amount_cents then 1.2 else 0.0 end,
          case when qt.q_amount_cents is not null and qt.q_amount_whole and pay.amount_cents / 100 = qt.q_amount_cents / 100 then 0.9 else 0.0 end
        ))::double precision as rank
    from public.payments pay
    left join public.clients c on c.id = pay.client_id and c.org_id = p_org
    left join public.invoices i on i.id = pay.invoice_id and i.org_id = p_org
    join query_terms qt on true
    where pay.org_id = p_org and pay.deleted_at is null
      and (
        lower(concat_ws(' ', c.first_name, c.last_name, c.company, c.email, i.invoice_number, pay.method, pay.provider)) ilike qt.pattern
        or (qt.q_amount_cents is not null and (
              pay.amount_cents = qt.q_amount_cents
              or (qt.q_amount_whole and pay.amount_cents / 100 = qt.q_amount_cents / 100)
            ))
      )
  ),
  lead_candidates as (
  ...
  leads_ranked as (
    select 'lead'::text as entity_type, lc.entity_id, lc.title, lc.subtitle, lc.extra_status,
      lc.extra_amount_cents, lc.extra_currency, lc.extra_date, lc.extra_client_id, lc.extra_client_name,
      null::uuid as extra_ref_id, lc.created_at, lc.rank
  ...
  invoices_ranked as (
    select 'invoice'::text as entity_type, i.id as entity_id,
      ...
      coalesce(nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(trim(c.company), ''), 'Unknown') as extra_client_name,
      null::uuid as extra_ref_id, i.created_at,
      (ts_rank_cd(to_tsvector('simple', lower(concat_ws(' ', i.invoice_number, i.subject, c.first_name, c.last_name, c.company, c.email))), qt.tsq) * 2.0
        + greatest(
            similarity(lower(coalesce(i.invoice_number, '')), qt.q),
            similarity(lower(concat_ws(' ', c.first_name, c.last_name, c.company)), qt.q),
            case when qt.q_amount_cents is not null and i.total_cents = qt.q_amount_cents then 1.0 else 0.0 end
          ))::double precision as rank
    ...
    where i.org_id = p_org and i.deleted_at is null
      and (
        ...
        or (qt.q_amount_cents is not null and (
              i.total_cents = qt.q_amount_cents
              or (qt.q_amount_whole and i.total_cents / 100 = qt.q_amount_cents / 100)
            ))
      )
  ),
  quotes_ranked as (
    select 'quote'::text as entity_type, q2.id as entity_id,
      ...
      coalesce(nullif(trim(concat_ws(' ', c.first_name, c.last_name)), ''), nullif(trim(c.company), ''),
        nullif(trim(concat_ws(' ', l.first_name, l.last_name)), ''), 'Unknown') as extra_client_name,
      null::uuid as extra_ref_id, q2.created_at,
  ...
  requests_ranked as (
    select 'request'::text as entity_type, fs.id as entity_id,
      ...
      coalesce(nullif(trim(concat_ws(' ', fs.first_name, fs.last_name)), ''), 'Unknown') as extra_client_name,
      null::uuid as extra_ref_id, fs.created_at,
  ...
  teams_ranked as (
    select 'team'::text as entity_type, t.id as entity_id, t.name as title, null::text as subtitle,
      null::text as extra_status, null::integer as extra_amount_cents, null::text as extra_currency,
      null::text as extra_date, null::uuid as extra_client_id, null::text as extra_client_name,
      null::uuid as extra_ref_id, t.created_at,
  ...
  events_ranked as (
    select 'event'::text as entity_type, se.id as entity_id,
      ...
      se.start_time::text as extra_date, j.client_id as extra_client_id, j.client_name as extra_client_name,
      j.id as extra_ref_id, se.created_at,
  ...
  union all select * from properties_ranked
  union all select * from jobs_ranked
  union all select * from agreements_ranked
  union all select * from payments_ranked
...
as $$
  select
    s.entity_type, s.entity_id, s.title, s.subtitle, s.extra_status,
    s.extra_amount_cents, s.extra_currency, s.extra_date,
    s.extra_client_id, s.extra_client_name, s.extra_ref_id,
    s.created_at, s.rank
...
create function public.search_global_by_type(
  p_org uuid,
  p_q text,
  p_entity_type text,
  p_limit int default 20,
...
create function public.search_global_counts(p_org uuid, p_q text)
returns table (
  entity_type text,
  total bigint
)
