...
if (!S.note && S.client) S.note = trouver(await lire('list_notes', { entity_type: 'client', entity_id: S.client }), 'Note exec', 'note_id', 'id');
await ex('update_note', () => S.note && { note_id: S.note, text: 'Note exec modifiée' });
await ex('delete_note', () => S.note && { note_id: S.note });
await ex('create_property', () => S.client && { client_id: S.client, name: 'Exec chalet', address: '2 chemin Exec', city: 'Magog', province: 'QC', country: 'CA', kind: 'service' }, (r) => { S.prop = trouver(r, null, 'property_id', 'id'); });
if (!S.prop && S.client) S.prop = trouver(await lire('list_properties', { client_id: S.client }), 'Exec chalet', 'property_id', 'id');
...
await ex('set_custom_field', () => S.champ && S.client && { field_id: S.champ, record_id: S.client, value: 'exec' });
...
if (!S.job) S.job = trouver(await lire('list_jobs', { query: 'Exec job', limit: 5 }), 'Exec job', 'job_id', 'id');
...
await ex('add_visit', () => S.job && { job_id: S.job, start_at: heure(5, 13), end_at: heure(5, 15) }, (r) => { S.visite = trouver(r, null, 'visit_id', 'event_id'); });
...
await ex('save_job_billing_milestones', () => S.job && { job_id: S.job, milestones: [{ id: 'm1', label: 'Dépôt', percent: 30 }, { id: 'm2', label: 'Solde', percent: 70 }], billing_split: true }, (r) => { S.jalon = trouver(r, 'Dépôt', 'milestone_id', 'id'); });
await ex('create_invoice_for_milestone', () => S.job && S.jalon && { job_id: S.job, milestone_id: S.jalon });
await ex('create_invoice_for_visit', () => S.job && S.visite && { job_id: S.job, visit_id: S.visite });
...
if (!S.recurrence && S.job) S.recurrence = trouver(await lire('list_recurrence_rules'), 'Exec job', 'rule_id', 'id');
...
await ex('create_invoice_from_job', () => S.job && { job_id: S.job }, (r) => { S.factureJob = trouver(r, null, 'invoice_id', 'id'); });
...
await ex('convert_quote_to_job', () => S.devis && { quote_id: S.devis, scheduled_at: heure(8, 9) });
...
await ex('create_quote_preset', { name: 'Exec préréglage', services: [{ name: 'Lavage', quantity: 1 }] }, (r) => { S.preset = trouver(r, null, 'preset_id', 'id'); });
if (!S.preset) S.preset = trouver(await lire('list_quote_presets'), 'Exec préréglage', 'preset_id', 'id');
...
await ex('create_quote_template', { name: 'Exec modèle devis', services: [{ name: 'Lavage', quantity: 1, unit_price_cents: 15000 }] }, (r) => { S.mdevis = trouver(r, null, 'template_id', 'id'); });
if (!S.mdevis) S.mdevis = trouver(await lire('list_quote_templates'), 'Exec modèle devis', 'template_id', 'id');
...
await ex('record_invoice_payment', () => S.facture && { invoice_id: S.facture, amount_cents: 2500, method: 'cash' });
await ex('mark_invoice_paid', () => S.facture && { invoice_id: S.facture, method: 'cash' });
...
await ex('create_recurring_invoice', () => S.client && { client_id: S.client, subject: 'Exec récurrente', items: [{ description: 'Entretien', qty: 1, unit_price_cents: 5000 }], frequency: 'monthly', start_date: jour(10) }, (r) => { S.recurrente = trouver(r, null, 'schedule_id', 'id'); });
if (!S.recurrente) S.recurrente = trouver(await lire('list_recurring_invoices'), 'Exec récurrente', 'schedule_id', 'id');
...
await ex('create_invoice_template', { name: 'Exec modèle facture', line_items: [{ description: 'Entretien', qty: 1, unit_price_cents: 5000 }] }, (r) => { S.mfact = trouver(r, null, 'template_id', 'id'); });
if (!S.mfact) S.mfact = trouver(await lire('list_invoice_templates'), 'Exec modèle facture', 'template_id', 'id');
...
await ex('create_checklist_template', { name: 'Exec liste', items: [{ id: 'x', type: 'checkbox', label: 'Exec', required: false }] }, (r) => { S.mliste = trouver(r, null, 'template_id', 'id'); });
if (!S.mliste) S.mliste = trouver(await lire('list_checklist_templates'), 'Exec liste', 'template_id', 'id');
...
await ex('create_email_template', { name: 'Exec courriel', type: 'generic', subject: 'Exec', body: 'Exec corps' }, (r) => { S.mcourriel = trouver(r, null, 'template_id', 'id'); });
if (!S.mcourriel) S.mcourriel = trouver(await lire('list_email_templates'), 'Exec courriel', 'template_id', 'id');
...
await ex('create_team', { name: 'Exec équipe', color_hex: '#22c55e' }, (r) => { S.equipe = trouver(r, null, 'team_id', 'id'); });
if (!S.equipe) S.equipe = trouver(await lire('list_teams'), 'Exec équipe', 'team_id', 'id');
...
if (!S.invit) S.invit = trouver(await lire('list_invitations', { status: 'all' }), 'exec.invite', 'invitation_id', 'id');
...
await ex('mark_conversation_read', () => S.conversation && { conversation_id: S.conversation });
...
await ex('create_tax_config', { name: 'Exec taxe', rate: 1.5, region: 'QC', country: 'CA' }, (r) => { S.taxe = trouver(r, null, 'tax_id', 'id'); });
if (!S.taxe) S.taxe = trouver(await lire('get_tax_config'), 'Exec taxe', 'tax_id', 'id');
...
await ex('create_service', { name: 'Exec service', price_cents: 1000 }, (r) => { S.service = trouver(r, null, 'service_id', 'id'); });
if (!S.service) S.service = trouver(await lire('list_services'), 'Exec service', 'service_id', 'id');
...
if (!S.rapport) S.rapport = trouver(await lire('list_scheduled_reports'), 'exec.rapport', 'report_id', 'id');
...
// ── Terrain, formations ──
await ex('create_house', { address: '9 rue Exec', lat: 45.51, lng: -73.56, status: 'unknown' }, (r) => { S.maison = trouver(r, null, 'house_id', 'id'); });
if (!S.maison) S.maison = trouver(await lire('list_houses', { query: 'Exec' }), '9 rue Exec', 'house_id', 'id');
...
await ex('create_territory', { name: 'Exec territoire', polygon: [[-73.6, 45.5], [-73.6, 45.51], [-73.59, 45.51], [-73.59, 45.5]] }, (r) => { S.territoire = trouver(r, null, 'territory_id', 'id'); });
if (!S.territoire) S.territoire = trouver(await lire('list_territories'), 'Exec territoire', 'territory_id', 'id');
...
await ex('start_field_session', { latitude: 45.5, longitude: -73.56 }, (r) => { S.session = trouver(r, null, 'session_id', 'id'); });
await ex('pause_field_session', () => ({ ...(S.session ? { session_id: S.session } : {}) }));
await ex('resume_field_session', () => ({ ...(S.session ? { session_id: S.session } : {}) }));
await ex('end_field_session', () => ({ latitude: 45.5, longitude: -73.56, ...(S.session ? { session_id: S.session } : {}) }));
