...
describe.skipIf(!DISPONIBLE)('T7 — isolation multi-tenant (staging)', () => {
  ...
  it('T7.1 — ROUGE ATTENDU (F1) : A ne peut pas faire traiter la visite de B', async () => {
  ...
  it('T7.3 — ROUGE ATTENDU (F1) : resolveEntityVariables(org A, visite de B) ne rend rien de B', async () => {
    const { resolveEntityVariables } = await import('../../../server/lib/actions');
  ...
  it('T7.4 — ROUGE ATTENDU (F1) : idem pour un job, un client, un lead de B', async () => {
    const { resolveEntityVariables } = await import('../../../server/lib/actions');
  ...
  it('T7.2 — ROUGE ATTENDU (F1) : les autres routes refusent aussi une entité de B', async () => {
    const jeton = await banc.jetonDe(A.owner.email);
  ...
  it('T7.9 — ROUGE ATTENDU (F4, migration M1) : un technicien de A ne peut pas écrire les tables du moteur', async () => {
