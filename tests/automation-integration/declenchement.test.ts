...
describe.skipIf(!DISPONIBLE)('T1 — déclenchement (staging)', () => {
  ...
  it('T1.3 — corrigé (F12/F3) : le même hook rejoué une heure plus tard ne renvoie pas de confirmation', async () => {
    // Une visite NEUVE : celle de T1.4 a déjà reçu sa confirmation, et l'idempotence
    // (F3, 24 h) refuserait à bon droit de la renvoyer — ce qui fausserait la mesure.
    const dans3jours = new Date(Date.now() + 3 * 86400_000); dans3jours.setUTCHours(15, 0, 0, 0);
    const { data: visite, error } = await banc.admin.from('schedule_events').insert({
      org_id: A.id, created_by: A.owner.id, job_id: A.job, title: 'Visite T1.3',
      start_at: dans3jours.toISOString(), end_at: new Date(dans3jours.getTime() + 3600_000).toISOString(),
    }).select('id').single();
    if (error) throw new Error(error.message);
    const corps = { eventId: visite.id, jobId: A.job, clientId: A.client.id };
