...
describe.skipIf(!DISPONIBLE)('T9 — le temps (staging)', () => {
  ...
  it('T9.8 — corrigé (F16) : règle désactivée pendant l’attente → la tâche ne part pas', async () => {
  ...
  it('T9.9 — corrigé (F16) : texte modifié pendant l’attente → le nouveau texte part', async () => {
    const { processScheduledTasks } = await import('../../server/lib/automationEngine');
    const regle = A.regles.get('job_reminder_2h')!;
  ...
  it('T9.10b — corrigé (F16) : job supprimé pendant l’attente → tâche annulée, jamais de « Bonjour , »', async () => {
    const { processScheduledTasks } = await import('../../server/lib/automationEngine');
