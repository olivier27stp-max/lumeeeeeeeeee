...
describe.skipIf(!DISPONIBLE)('T11 — conformité (staging)', () => {
  ...
  it('T11.4 — corrigé (F26) : désabonnement courriel après la planification → refus à l’exécution, sans reprise', async () => {
    const { processScheduledTasks } = await import('../../server/lib/automationEngine');
  ...
  it('T11.7 — corrigé (F8, M3) : le log d’un envoi porte le destinataire, l’acteur, l’événement et l’instantané de la règle', async () => {
    const { processScheduledTasks } = await import('../../server/lib/automationEngine');
  ...
  it('T11.8 — corrigé (F21, M6) : purge_automation_history(12) efface les journaux de plus de 12 mois et garde les récents', async () => {
