...
describe.skipIf(!DISPONIBLE)('T4 — idempotence et reprise (staging)', () => {
  ...
  it('T4.2 — corrigé (F3) : le même hook posté deux fois → un seul SMS de confirmation', async () => {
  ...
  it('T4.6 — deux dépilages concurrents sur la même tâche échue → un seul envoi', async () => {
    ...
    // F16 : le texte parti est celui de la RÈGLE au moment de l'envoi, pas la copie de la tâche — on compte par destinataire.
    expect(sms.filter((s) => s.to === A.client.phone).length).toBe(1);
  }, 30_000);
  ...
  it('T4.4 — corrigé (F5) : une tâche reprise après un crash dont l’exécution est déjà journalisée réussie ne renvoie pas', async () => {
    const { processScheduledTasks } = await import('../../server/lib/automationEngine');
