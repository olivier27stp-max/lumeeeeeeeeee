...
describe('T9.1 — l’heure de rendez-vous écrite au client est celle de Montréal, quel que soit le fuseau du serveur', () => {
  ...
  it('F2 corrigé : serveur en UTC (Railway) → « 09:00 », pas « 13:00 »', async () => {
    process.env.TZ = 'UTC';
  ...
  it('F2 corrigé : un rendez-vous à 22 h à Montréal ne change pas de DATE sur un serveur UTC', async () => {
    process.env.TZ = 'UTC';
    const { resolveEntityVariables } = await import('../../server/lib/actions');
...
describe('T9.2 — rappel « X jours avant » à cheval sur un changement d’heure', () => {
  it('corrigé (F14) : rendez-vous le 2 nov. 2026 à 10 h (heure normale) → rappel J-7 le 26 oct. à 10 h locale, pas 11 h', async () => {
...
describe('T9.5 — un rappel « avant » que les heures calmes repousseraient APRÈS le rendez-vous est abandonné', () => {
  it('corrigé : rappel 2 h avant un rendez-vous à 7 h, échu à 5 h → annulé, pas envoyé à 8 h (une heure après le passage)', async () => {
    vi.useFakeTimers({ toFake: ['Date'] });
    ...
    expect(derniere.status === 'cancelled' || !repousseApres, `rappel repoussé à ${derniere.execute_at} (${derniere.execute_at ? heureMontreal(derniere.execute_at) : '—'}) pour un rendez-vous à 07:00`).toBe(true);
    expect(derniere.last_error).toMatch(/heures calmes/);
