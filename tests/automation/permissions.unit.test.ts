...
describe('T8.3 — destinataire arbitraire dans une règle', () => {
  it('F18 corrigé : `config.to` est ignoré, le SMS part au client de l’entité', async () => {
  ...
  it('F18 corrigé : `config.to` templatable — « [client_phone] » ou une adresse externe — n’est pas honoré non plus pour le courriel', async () => {
...
describe('T8.5 — contexte d’exécution : « système », jamais l’acteur', () => {
  ...
  it('l’acteur de l’événement est écrit dans activity_log (bus) ET dans la trace d’exécution (F8 corrigé, M3)', async () => {
    ...
    expect(log.actor_id).toBe(ACTEUR);
    expect(log.rule_snapshot?.action?.type).toBe('log_activity');
  });
});
