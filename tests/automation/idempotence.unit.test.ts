...
const donneesA = () => ({
  ...
  jobs: { data: { title: 'Gouttières', client_id: 'client-a', status: 'scheduled', deleted_at: null } },
...
/**
 * File de tâches SIMULÉE (l'enregistreur est sans état) : la réservation d'une
 * action immédiate (F3) s'y inscrit, et la lecture « déjà exécutée ? » la
 * retrouve. C'est le comportement de l'index unique + de la ligne `completed`
 * que l'intégration T4.2 éprouve contre la vraie base.
 */
const fileSimulee = () => {
  const reservees = new Set<string>();
  return (req: any) => {
    if (req.op === 'insert') {
      const cle = req.valeur?.execution_key as string;
      if (reservees.has(cle)) return { data: null, error: { code: '23505', message: 'duplicate key' } };
      reservees.add(cle);
      return { data: { id: `res-${reservees.size}` } };
    }
    if (req.op === 'select') {
      const cle = req.filtres.find(([, col]: any) => col === 'execution_key')?.[2] as string | undefined;
      return { data: cle && reservees.has(cle) ? { id: 'res' } : null };
    }
    return { data: null };
  };
};
...
describe('T4.1 — même événement livré deux fois → un seul effet immédiat', () => {
  it('F3 corrigé : deux émissions identiques d’appointment.created → un seul SMS', async () => {
    const { eventBus, journal } = await moteur({ ...donneesA(), automation_rules: { data: [regleImmediate] }, automation_scheduled_tasks: fileSimulee() });
...
describe('T4.5 — reprise après échec du fournisseur', () => {
  ...
  // F5 : Twilio et Resend n'offrent pas de clé d'idempotence sur l'envoi d'un
  // message (vérifié dans leurs SDK). La garde est donc le journal d'exécution :
  // une reprise (attempts ≥ 1) dont l'exécution précédente est journalisée
  // RÉUSSIE (processus mort entre l'envoi et la clôture) clôt sans renvoyer.
  it('F5 corrigé : reprise après un arrêt brutal, exécution déjà journalisée réussie → clôturée, aucun renvoi', async () => {
    const { processScheduledTasks, client, journal } = await moteur({
      ...donneesA(),
      automation_scheduled_tasks: { data: [tache(1)] },
      automation_execution_logs: (req: any) => (req.op === 'select' ? { data: { id: 'log-ok' } } : { data: null }),
    });
    await processScheduledTasks(client);
    expect(twilio.messages.create).not.toHaveBeenCalled();
  ...
  it('témoin : reprise sans exécution réussie journalisée → renvoi (au plus une fois de trop, jamais zéro)', async () => {
    const { processScheduledTasks, client } = await moteur({ ...donneesA(), automation_scheduled_tasks: { data: [tache(1)] } });
    await processScheduledTasks(client);
