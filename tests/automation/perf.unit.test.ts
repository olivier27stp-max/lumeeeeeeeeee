...
async function coutDe(actions: Array<{ type: string; config: Record<string, unknown> }>) {
  const { initAutomationEngine, viderCachePlafonds } = await import('../../server/lib/automationEngine');
  ...
  viderCachePlafonds(); // le forfait est mis en cache 60 s : chaque mesure repart à froid
...
describe('cliquet — requêtes Supabase par événement immédiat', () => {
  // Mesuré après les correctifs de l'audit (2026-09-14). Socle 8 : bus, règles,
  // réglages (mémoïsés, F24), company_settings des variables, jobs, clients,
  // job_agreements ×2. Chaque action immédiate coûte désormais +2 (réservation
  // d'idempotence F3 : select + insert ; +1 update en base réelle quand la
  // réservation renvoie un id) ; le premier SMS/courriel d'une org lit le
  // forfait (M4 : subscriptions, puis plans si abonnement). Avant : sms +4,
  // email +5, request_review +15, welcome 21.
  const SOCLE = 8;
  const cas: Array<[string, Array<{ type: string; config: Record<string, unknown> }>, number]> = [
    ['aucune action (socle)', [], SOCLE],
    ['log_activity', [{ type: 'log_activity', config: { event_type: 'x' } }], SOCLE + 4],
    ['create_notification', [{ type: 'create_notification', config: { title: 't', body: 'b' } }], SOCLE + 4],
    ['create_task', [{ type: 'create_task', config: { title: 't' } }], SOCLE + 5],
    ['send_sms', [{ type: 'send_sms', config: { body: 'x' } }], SOCLE + 8],
    ['send_email', [{ type: 'send_email', config: { subject: 's', body: 'b' } }], SOCLE + 8],
    ['request_review', [{ type: 'request_review', config: {} }], SOCLE + 19],
    ['welcome (sms + email + notification + log)', [{ type: 'send_sms', config: { body: 'x' } }, { type: 'send_email', config: { subject: 's', body: 'b' } }, { type: 'create_notification', config: { title: 't', body: 'b' } }, { type: 'log_activity', config: { event_type: 'x' } }], SOCLE + 8 + 7 + 4 + 4],
  ];
