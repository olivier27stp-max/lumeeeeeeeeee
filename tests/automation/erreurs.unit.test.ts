...
describe('T10.3 — après un échec définitif, l’administrateur est prévenu', () => {
  it('corrigé : une notification à l’org existe avec le motif en clair', async () => {
    const m = monde(); (m.schedule_events.data.job.clients as any).phone = null;
    const { processScheduledTasks, client, journal } = await moteur({ ...m, automation_scheduled_tasks: { data: [tache('t', 0)] } });
    await processScheduledTasks(client);
    const fin = requetes(journal, 'automation_scheduled_tasks', 'update').map((r) => r.valeur as any).at(-1);
    expect(fin.status).toBe('failed');
...
describe('T10.4 — un fournisseur qui ne répond pas ne bloque pas le tick', () => {
  it('corrigé : sendEmail bloqué 40 s → l’action échoue en moins de 8 s et la tâche est reprise', async () => {
    process.env.AUTOMATION_ACTION_TIMEOUT_MS = '3000'; // délai court pour le test ; défaut 20 s
    ...
    delete process.env.AUTOMATION_ACTION_TIMEOUT_MS;
  }, 15_000);
});

describe('T10.5 — écritures multiples sans transaction (R6)', () => {
  it('corrigé (R6) : request_review dont le suivi (review_requests) est refusé ne doit pas se dire réussi', async () => {
...
describe('T10.7 — une règle qui lève ne fait pas sauter les règles suivantes du même événement', () => {
  it('corrigé : la 1re règle lève (lecture qui explose) → la 2e règle doit quand même s’exécuter', async () => {
...
describe('T10.9 — un événement émis avant l’initialisation du moteur', () => {
  it('ROUGE ATTENDU (F9, migration M8 « outbox » — deuxième vague, non livrée) : il est perdu — rien n’est persisté pour un traitement ultérieur', async () => {
    vi.resetModules();
    const { eventBus } = await import('../../server/lib/eventBus');
