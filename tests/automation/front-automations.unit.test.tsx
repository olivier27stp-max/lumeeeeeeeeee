...
describe('T13.6 — badge d’échec', () => {
  it('3 échecs sur 7 jours ⇒ badge « 3 échec » avec un title lisible', async () => {
    vi.mocked(api.getRecentAutomationFailures).mockResolvedValue([1, 2, 3].map((i) => ({ id: `l${i}`, automation_rule_id: RULE_ID, action_type: 'send_sms', result_error: 'No recipient phone', entity_type: 'schedule_event', created_at: '2026-09-13T15:00:00Z' })));
    await rendre();
...
describe('T13.7 — déclencheur et nom lisibles', () => {
  it('F22 corrigé : `agreement.signed` s’affiche brut au lieu de « Contrat signé »', async () => {
  ...
  it('F22 corrigé : le nom semé en anglais « Contract Signed » n’est pas traduit dans l’interface française', async () => {
...
describe('T13.8 — éditeur de message (SMS)', () => {
  ...
  it('F22 corrigé : une variable inconnue « [prenom] » n’est pas signalée — le serveur l’enverra vide, l’aperçu la montre telle quelle', async () => {
    await rendre(); await deplier();
...
describe('T13.10 — pourquoi ça n’a pas marché', () => {
  it('F22 corrigé : avec 2 échecs, aucune raison lisible n’est affichée nulle part (ni dans la ligne, ni dépliée)', async () => {
    vi.mocked(api.getRecentAutomationFailures).mockResolvedValue([
