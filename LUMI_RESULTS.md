...
- **Deuxième vague** (même jour) : réponses fixes (« merci », « ok », « salut » — un tour de modèle à 0,6 ¢ chacun avant), créations avec les champs dans la phrase (client, prospect, job chez un client unique avec date simple, note sur un client), textos et courriels dictés mot pour mot après « : », listes par client (jobs, factures, solde, devis), report d'un job par numéro avec date simple, « relance mes retards » avec le texte standard, revenus de l'année et des 30 derniers jours. Essai de bout en bout : **35 phrases sur 36 à 0 ¢**, la 36e étant l'ambiguïté voulue. Garde-fous : deux demandes dans la même phrase (« … et texte-lui ») → modèle ; un morceau qu'on ne sait pas ranger dans une fiche → modèle ; date non simple → modèle ; doublon de nom → modèle ; séparateur « : » ou tiret entouré d'espaces seulement (« Jean-Pierre » n'est pas un séparateur).
- Ce que ça change pour l'usage réel : les demandes conversationnelles restent au modèle, mais le pointage quotidien, les listes de réglages, les créations dictées, les messages dictés et les commandes par numéro ne coûtent plus rien.

### 7.8 Extraction par le routeur : Haiku lit, le code bâtit (2026-09-17)

Pour une écriture simple formulée naturellement (« fais-moi une job chez Linda Tremblay jeudi matin pour les vitres », « ajoute Linda comme cliente, son cell c'est 514-… », « rappelle-moi de commander du sel demain »), le routeur Haiku, déjà appelé sur chaque message et en cache, sort les champs (client, date, heure, titre, texte dicté, numéro) ; le code bâtit la même carte que pour les motifs stricts, avec les mêmes résolutions (client unique, doublon, date simple). Coût : **0,19 ¢ au lieu de 0,6 à 1,5 ¢**, et la carte reste à confirmer.

Garde-fou déterministe, `extractionFidele` : rien d'extrait ne doit être inventé ni réécrit. Un placeholder (« <UNKNOWN> ») ou un nom absent du message rend au modèle ; un texto ou un courriel n'est bâti que si ses mots sont dictés après « : », entre guillemets ou après « mot pour mot » (« texte à Linda qu'on arrive » a été réécrit par Haiku en « on arrive » → refusé → le modèle rédige). Deux demandes, une date chiffrée, un nom incomplet → pas d'extraction, par consigne et par garde.

Essai de bout en bout (`scripts/qa/essayer-extraction.mts`) : **12 sur 12** — 8 phrases naturelles en carte à 0,19 ¢ (une servie encore moins cher par un motif strict), 4 pièges au modèle. Un piège trouvé en route : « rappelle-moi de … » était classé comme la liste des tâches ; corrigé dans le prompt du routeur.
