...
const PIN_STATUSES: { label: string; color: string; house: HouseStatus }[] = [
  { label: 'Fermé ✓', color: '#22C55E', house: 'sale' },
  { label: 'Suivi', color: '#06B6D4', house: 'lead' },
  { label: 'Rendez-vous', color: '#6B7280', house: 'quote_sent' },
  { label: 'Pas de réponse', color: '#EAB308', house: 'no_answer' },
  { label: 'Refusé', color: '#EF4444', house: 'not_interested' },
  { label: 'Autre', color: '#9CA3AF', house: 'unknown' },
];
...
export default function D2DMap() {
  ...
  const saveZone = async () => {
    ...
    try {
      ...
      closeZone();
      refetchZones();
      Alert.alert('Zone', 'Zone enregistrée ✓');
    } catch (e) {
      Alert.alert('Zone', (e as Error).message);
    }
  };
    ...
    <View className="flex-1">
      ...
      >
        <Text className="text-sm font-medium text-ink">
          {houses?.length ?? 0} maisons · {zones?.length ?? 0} zones
        </Text>
      </View>
      ...
      {armed ? (
        <View className="absolute left-5 right-5 flex-row items-center gap-3" style={{ bottom: insets.bottom + 24 }}>
          <View className="flex-1 rounded-xl bg-white/90 px-4 py-3">
            <Text className="text-sm font-medium text-ink">Tape la maison sur la carte 📍</Text>
          </View>
          <Pressable
            onPress={() => {
              setArmed(null);
          ...
          >
            <Text className="text-sm font-semibold text-ink">Annuler</Text>
          </Pressable>
        </View>
      ) : null}
      ...
      {drawing ? (
        <View className="absolute left-5 right-5 flex-row items-center gap-3" style={{ bottom: insets.bottom + 24 }}>
          <View className="flex-1 rounded-xl bg-white/90 px-4 py-3">
            <Text className="text-sm font-medium text-ink">Tape les coins, puis « Terminer »</Text>
          </View>
          <Pressable
            onPress={() => {
        ...
        >
          <View className="mb-3 flex-row items-center justify-between">
            <Text className="text-base font-bold text-ink">
              {pickerMode === 'place' ? 'Choisis un statut, puis tape la carte' : 'Changer le statut'}
            </Text>
            <Pressable
              onPress={() => {
                setPickerMode(null);
          ...
          <ScrollView keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 14, paddingBottom: 4 }}>
            {pickerMode === 'edit' && editHouseId ? (
              ...
              >
                ...
                <Text className="text-center text-[11px] text-ink-muted">Fiche</Text>
              </Pressable>
            ) : null}
            {PIN_STATUSES.map((s) => (
              <Pressable key={s.house} onPress={() => onChip(s.house)} className="w-[72px] items-center gap-1">
                ...
                />
                <Text className="text-center text-[11px] text-ink-muted">{s.label}</Text>
              </Pressable>
            ))}
          </ScrollView>
        </View>
      ...
      <Modal visible={!!zoneCoords} transparent animationType="slide" onRequestClose={closeZone}>
        <Pressable className="flex-1 justify-end bg-black/40" onPress={closeZone}>
          <Pressable className="gap-3 rounded-t-3xl bg-white p-5" onPress={(e) => e.stopPropagation()}>
            <Text className="text-lg font-bold text-ink">Nouvelle zone</Text>
            <TextInput
              value={zoneName}
              onChangeText={setZoneName}
              placeholder="Nom de la zone (ex. Secteur Nord)"
              className="rounded-xl border border-surface-border px-4 py-3 text-base text-ink"
            />
            <Text className="text-xs font-semibold uppercase tracking-wide text-ink-subtle">Assigner à une équipe</Text>
            <View className="flex-row flex-wrap gap-2">
              ...
              >
                <Text className={`text-sm font-medium ${zoneTeam === null ? 'text-white' : 'text-ink'}`}>Aucune</Text>
              </Pressable>
              {(teams ?? []).map((t) => {
            ...
            >
              <Text className="text-base font-semibold text-white">Enregistrer la zone</Text>
            </Pressable>
            <Pressable onPress={closeZone} className="items-center py-1">
              <Text className="text-sm font-medium text-ink-muted">Annuler</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
