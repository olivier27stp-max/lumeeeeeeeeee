...
- **Moteur d'automatisations** (`server/lib/automationEngine.ts`, `actions/index.ts`, routes `automation-events.ts` — audit 2026-09-13, correctifs 2026-09-14) : toute lecture sous `service_role` porte `.eq('org_id', …)` (seul garde-fou entre tenants) ; les routes de hooks vérifient l'appartenance de l'entité avant d'émettre (404 sinon) ; une action immédiate réserve d'abord une ligne `automation_scheduled_tasks` (`running`, clé `règle:entité:index`, index unique) = idempotence ; le tick relit la règle (`is_active`, `actions`) au moment d'exécuter ; le destinataire est TOUJOURS le client de l'entité (`config.to` ignoré) ; dates/heures formatées dans `company_settings.timezone`, jamais dans le fuseau du serveur. Interrupteurs : `AUTOMATIONS_ENABLED=false` (global, env), `company_settings.automations_paused_at` / `automations_dry_run` (par org). Tests : `tests/automation/*` (unitaires, client enregistreur) ; `AUTOMATIONS_IT=1 npx vitest run tests/automation-integration/ --no-file-parallelism` (staging, base partagée → JAMAIS en parallèle). Golden set des 35 presets : `GOLDEN_UPDATE=<preset_key>` pour régénérer sciemment.
