...
- **Lumi (assistant IA)** : les consignes de présentation vivent dans `server/lib/agent/consignesCollegue.ts` (partagées avec le MCP) ; `npm run qa:lumi` (API locale sur staging, ≈ 1,60 $ d'inférence par passe) envoie 53 demandes en français et mesure le taux d'erreur contre la base — à relancer après tout changement de prompt ou d'outil. Référence au 2026-09-10 : 96 % (2 échecs = correcteur, pas Lumi).
