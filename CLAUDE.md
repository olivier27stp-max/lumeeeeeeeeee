...
- **`supabase/baseline/`** is the source of truth to create a NEW environment — the 400 files in `supabase/migrations/` do **not** rebuild the DB from scratch (incomplete history; replaying them on an empty DB fails at the 2nd file). `npm run db:bootstrap` applies the baseline. Regenerated and validated 2026-09-25 by replaying it on a blank Supabase Postgres 17 container: 254 tables, 15 views, 407 functions, 675 policies, zero errors. Regenerate it after any structural change: `node --env-file=.env.local scripts/regenerer-baseline.mjs` (see `supabase/baseline/README.md`). Run it when in doubt, or after someone else deployed.
...
- **Support en deux niveaux** (2026-09-15) : `SupportChat` → `POST /api/support/chat` → assistant Claude (`server/lib/support/ia.ts`, Sonnet 5, doc `search_help` + FAQ `src/components/supportArticles.ts`, copiée dans l'image Docker) ; transfert humain = ticket `support_tickets` + **un canal Slack par entreprise cliente** (`server/lib/support/canaux-slack.ts`, table `support_slack_channels`, scope `channels:manage` ; repli : fil dans #support ; archivé après 7 jours sans demande, `SLACK_ARCHIVE_APRES_JOURS`, désarchivé à la prochaine) ; dans ce canal l'équipe écrit au premier niveau et ça part au dernier ticket ouvert de l'entreprise (`server/lib/slack.ts`, `SLACK_BOT_TOKEN/SIGNING_SECRET/SUPPORT_CHANNEL_ID`) ; les réponses du fil reviennent par `POST /api/webhooks/slack` (corps brut, signature v0, monté avant express.json) ET par le relevé périodique `server/lib/support/relais-slack.ts` (`conversations.replies` toutes les 45 s, `SLACK_POLL_MS`), qui ne dépend pas de l'abonnement aux événements ; chaque appel du webhook laisse une trace dans `webhook_receipts` → message `agent` + notification + courriel au client. Sans Slack : courriel à `SUPPORT_EMAIL` comme avant. Un autre bot dans le fil (Grok) est relayé comme un humain ; seul NOTRE bot est ignoré.
