...
export function parseTab(raw: unknown): SearchTab {
  ...
  if (value === 'clients' || value === 'properties' || value === 'jobs' || value === 'agreements' || value === 'payments' || value === 'leads' || value === 'invoices' || value === 'quotes' || value === 'requests' || value === 'teams' || value === 'events') return value;
  return 'all';
}
...
export function parseCountRows(rows: Array<{ entity_type: SearchEntityType; total: number }> | null | undefined) {
  const base = { clients: 0, properties: 0, jobs: 0, agreements: 0, payments: 0, leads: 0, invoices: 0, quotes: 0, requests: 0, teams: 0, events: 0 };
  for (const row of rows || []) {
    ...
    if (row.entity_type === 'payment') base.payments = total;
  ...
  return {
    ...base,
    all: base.clients + base.properties + base.jobs + base.agreements + base.payments + base.leads + base.invoices + base.quotes + base.requests + base.teams + base.events,
  };
}
