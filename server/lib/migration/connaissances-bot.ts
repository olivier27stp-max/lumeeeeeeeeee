...
7. Champs personnalisés Jobber « CFT[...] » / « CFL[...] », colonnes « Receives automatic ... », « Text Message Enabled Phone » → null (notes). Statut client : l'import force « actif », « Archived » → null.
8. Adresses : le champ « address » d'un CLIENT est son adresse de SERVICE (elle crée le pin sur la carte). L'adresse de facturation est une entité SÉPARÉE (billing_property) qui vient d'un fichier séparé : dans un fichier clients, les colonnes « Billing ... » → null. « Street 2 » → null.
...
13. Dépôt / « Deposit » sur une facture → null (déjà reflété dans « balance »). « balance » fait foi pour payé / partiel / impayé ; « paid_amount » sert seulement sans solde.
...
export const SEMANTIQUE_ENTITE: Record<TargetEntity, string> = {
  client: `client : first_name/last_name (nom de la personne) ; full_name n'est utilisé que si prénom ET nom sont vides ; company = raison sociale (texte, jamais un oui/non) ; email = courriel unique (clé de dédup et de rattachement ; plusieurs courriels dans une cellule = invalide) ; phone = téléphone principal (clé de rattachement sur 10 chiffres) ; phone_secondary = second numéro ; address/city/province/postal_code/country = adresse de SERVICE (pin carte) ; lead_source = source du lead ; notes = notes libres ; status = IGNORÉ à l'import (tout client arrive actif) ; external_id = identifiant de l'ancien CRM (J-ID, Customer ID) ; created_date = date de création d'origine (préservée).`,
  property: `property : address (obligatoire, adresse de service d'une propriété du client) + city/province/postal_code/country ; client_ref / client_email_ref / client_name_ref / client_phone_ref = client propriétaire ; name = nom de la propriété ; notes.`,
  billing_property: `billing_property : address (obligatoire, adresse de FACTURATION, une seule par client) + city/province/postal_code/country ; client_ref (obligatoire) ou client_email_ref / client_name_ref / client_phone_ref ; notes.`,
  ...
  invoice: `invoice (facture) : invoice_number ; status ; issued_date ; due_date ; subtotal = avant taxes ; discount = rabais soustrait avant taxes ; tax = montant des taxes ; total = total TTC du document ; paid_amount ; balance = solde (fait foi) ; client_ref + client_email_ref / client_name_ref / client_phone_ref ; job_ref ; notes ; salesperson ; created_date ; external_id. Dépôt → null. Les lignes (services facturés) ne sont PAS importées : elles vont dans un fichier séparé « Lignes » (manque connu).`,
