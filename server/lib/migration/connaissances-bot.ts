...
export const SEMANTIQUE_ENTITE: Record<TargetEntity, string> = {
  ...
  invoice: `invoice (facture) : invoice_number ; status ; issued_date ; due_date ; subtotal = avant taxes ; discount = rabais soustrait avant taxes ; tax = montant des taxes ; total = total TTC du document ; paid_amount ; balance = solde (fait foi) ; paid_date = date de paiement (factures soldées) ; client_ref + client_email_ref / client_name_ref / client_phone_ref ; job_ref ; notes ; salesperson ; created_date ; external_id ; line_items = « Line items » (Jobber : « Nom (qté, $total de la ligne), … »). Dépôt → null. Chaque facture importée reçoit ses lignes : celles de line_items si leur somme égale le sous-total, sinon une ligne « Montant importé ».`,
