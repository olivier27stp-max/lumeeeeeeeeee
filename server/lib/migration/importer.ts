...
export async function runFinalImport(
  ...
  for (const entity of entities) {
    ...
    if (entity === 'quote' && importedIds.length > 0) {
      const importes = new Set(importedIds);
      const r = await creerLignesDevisImportees(admin, migration.org_id, toInsert.filter((c) => importes.has(c.rec.id)).map((c) => ({
        id: c.id,
        lignes: lignesPourFacture(texteLignes(c.rec.normalized ?? {}), Number(c.row.subtotal_cents) || 0),
      })));
      if (r.echecs > 0) console.error(`[migration-importer] ${r.echecs} soumission(s) sans lignes après import`);
    }
