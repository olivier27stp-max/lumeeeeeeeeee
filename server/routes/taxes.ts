...
// Catalogue des presets de taxes : source unique dans server/lib/seedOrgDefaults.ts
// (partagé avec le seeding d'onboarding serveur — webhook Stripe, /checkout).
...
router.post('/taxes/setup', validate(setupTaxSchema), async (req, res) => {
  try {
    ...
    // La création (configs → group → items → default) vit dans seedTaxPreset,
    // partagée avec le seeding d'onboarding serveur. Ici on garde seulement
    // les codes HTTP (400/409) que le seeder ne distingue pas.
    await seedTaxPreset(admin, auth.orgId, preset_key, !!make_default);
    const { data: group } = await admin
      .from('tax_groups')
      .select('*')
      .eq('org_id', auth.orgId)
    ...
    return res.json({ group, config_count: preset.taxes.length });
  } catch (err: any) {
