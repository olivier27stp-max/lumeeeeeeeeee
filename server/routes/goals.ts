...
router.post('/goals', validate(createGoalSchema), async (req, res) => {
  try {
    ...
    if (error) throw error;
    cacheDeletePrefix(`goals:progress:${auth.orgId}`);
...
// GET /api/goals/progress — get actual values for active goals (cached 60s, parallelized)
router.get('/goals/progress', async (req, res) => {
  try {
    ...
    const payload = await cached(`goals:progress:${auth.orgId}`, 60, async () => {
      ...
      // Compute all goals' actual values in parallel
