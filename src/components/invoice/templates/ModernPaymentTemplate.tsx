    ...
    <div className="bg-[#f8fafc] text-[#111]" style={{ fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI","Inter",sans-serif', fontSize: '13px', lineHeight: 1.55 }}>
      ...
      <div className="px-8 py-8 space-y-5">
        ...
        <div className="rounded-xl overflow-hidden border-2" style={{ borderColor: isPaid ? '#22c55e' : ACCENT }}>
          <div className="px-6 py-5 text-white" style={{ backgroundColor: isPaid ? '#22c55e' : ACCENT }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-widest opacity-80">{isPaid ? (fr ? 'Payée en totalité' : 'Paid in full') : (fr ? 'Montant dû' : 'Amount due')}</p>
              ...
              <div className="text-right text-[11px] opacity-80">
                <p>{fr ? 'Échéance :' : 'Due:'} {fmtDate(data.due_date, locale)}</p>
                <p>{fr ? 'Émise le' : 'Issued:'} {fmtDate(data.issued_at || data.created_at, locale)}</p>
              </div>
            </div>
          </div>
          {data.paid_cents > 0 && !isPaid && (
            <div className="bg-white px-6 py-3 flex justify-between text-[12px]">
              <span className="text-[#6b7280]">{fr ? 'Total :' : 'Total:'} {fmt(data.total_cents)} \u00b7 {fr ? 'Pay\u00e9 :' : 'Paid:'} <span className="text-[#15803d] font-medium">{fmt(data.paid_cents)}</span></span>
              <span className="font-semibold">{fr ? 'Restant :' : 'Remaining:'} {fmt(balance)}</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] p-5">
          <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: ACCENT }}>{fr ? 'Facturer à' : 'Bill to'}</p>
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] overflow-hidden">
          <div className="px-5 py-3 border-b border-[#f3f4f6]">
            <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: ACCENT }}>{fr ? 'Articles' : 'Items'}</p>
          </div>
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#f3f4f6] bg-[#fafafa]">
                ...
                <th className="px-3 py-2.5 text-right text-[10px] font-medium uppercase tracking-widest text-[#9ca3af] w-14">{fr ? 'Qté' : 'Qty'}</th>
                <th className="px-3 py-2.5 text-right text-[10px] font-medium uppercase tracking-widest text-[#9ca3af] w-24">{fr ? 'Prix' : 'Price'}</th>
                <th className="px-5 py-2.5 text-right text-[10px] font-medium uppercase tracking-widest text-[#9ca3af] w-24">{fr ? 'Montant' : 'Amount'}</th>
              </tr>
            </thead>
            <tbody>
              {data.items.map(item => (
          ...
          <div className="px-5 py-4 bg-[#fafafa]">
            <div className="flex justify-end">
              <div className="w-56 text-[12px] space-y-1.5">
                <div className="flex justify-between"><span className="text-[#9ca3af]">{fr ? 'Sous-total' : 'Subtotal'}</span><span className="tabular-nums">{fmt(data.subtotal_cents)}</span></div>
                {data.discount_cents > 0 && <div className="flex justify-between text-[#dc2626]"><span>{fr ? 'Rabais' : 'Discount'}</span><span className="tabular-nums">-{fmt(data.discount_cents)}</span></div>}
                <div className="flex justify-between"><span className="text-[#9ca3af]">{fr ? 'Taxes' : 'Tax'}</span><span className="tabular-nums">{fmt(data.tax_cents)}</span></div>
