...
export default function FieldEstimateTemplate({ data }: { data: QuoteRenderData }) {
  ...
  const L = fr ? {
    estimate: 'Soumission',
    customer: 'Client',
    jobDetails: 'Détails des travaux',
    date: 'Date',
    ...
    notes: 'Notes',
    workTerms: 'Autorisation de travaux et conditions',
    ...
    notes: 'Notes',
    workTerms: 'Work Authorization & Terms',
      ...
      <div className="px-8 py-6">
        ...
        <div className="flex items-start justify-between gap-8">
          ...
          <div className="text-right">
            <div className="inline-block rounded-lg px-5 py-3" style={{ backgroundColor: dark }}>
              <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400">{L.estimate}</p>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          ...
          <div className="rounded-lg border border-[#e5e7eb] p-4">
            <p className="text-[9px] font-bold uppercase tracking-wider text-[#9ca3af]">{L.customer}</p>
          ...
          <div className="rounded-lg bg-[#f8fafc] border border-[#e2e8f0] p-4">
            <p className="text-[9px] font-bold uppercase tracking-wider text-[#9ca3af]">{L.jobDetails}</p>
            <p className="mt-2 text-[14px] font-semibold text-[#111]">{data.title}</p>
            <div className="mt-2 grid grid-cols-2 gap-y-1.5 text-[12px]">
              <span className="text-[#9ca3af]">{L.date}</span>
              <span className="text-[#374151] font-medium">{fmtDate(data.created_at, locale)}</span>
              {data.valid_until && <>
                <span className="text-[#9ca3af]">{L.validUntil}</span>
                <span className="text-[#374151] font-medium">{fmtDate(data.valid_until, locale)}</span>
              </>}
              <span className="text-[#9ca3af]">{L.status}</span>
              <span className="font-semibold" style={{
                color: data.status === 'approved' ? '#059669' : data.status === 'declined' ? '#dc2626' : '#374151',
              }}>
                {fr
                  ? (STATUS_FR[data.status] || STATUS_FR.draft)
        ...
        {data.introduction && (
          <div className="mt-6 rounded-lg border-l-4 bg-[#f8fafc] px-5 py-4" style={{ borderColor: dark }}>
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#9ca3af]">{L.scope}</p>
        ...
        {data.optional_items.length > 0 && (
          <div className="mt-4 rounded-lg border border-dashed border-[#d1d5db] overflow-hidden">
            <div className="px-5 py-2.5 bg-[#fefce8] border-b border-[#fde68a] flex items-center justify-between">
              <p className="text-[11px] font-bold uppercase tracking-wider text-[#92400e]">{L.optionalServices}</p>
            </div>
            <table className="w-full">
              <tbody>
          ...
          <div className="w-80 rounded-lg border border-[#e5e7eb] overflow-hidden">
            ...
            {data.deposit_required && data.deposit_cents > 0 && (
              <div className="flex justify-between px-5 py-3 text-[12px] bg-[#eff6ff] border-t border-[#bfdbfe]">
                <span className="text-[#1d4ed8] font-medium">{L.deposit}</span>
        ...
        {data.notes && (
          <div className="mt-8 rounded-lg border border-[#e5e7eb] bg-[#f9fafb] p-5">
            <p className="text-[9px] font-bold uppercase tracking-wider text-[#9ca3af]">{L.notes}</p>
        ...
        {data.contract_disclaimer && (
          <div className="mt-4 rounded-lg border border-[#e5e7eb] bg-[#f9fafb] p-5">
            <p className="text-[9px] font-bold uppercase tracking-wider text-[#9ca3af]">{L.workTerms}</p>
        ...
        <div className="mt-8 border-t border-[#e5e7eb] pt-6">
          <p className="text-[10px] font-bold uppercase tracking-wider text-[#9ca3af] mb-6">{L.authorization}</p>
          <div className="grid grid-cols-2 gap-8">
            <div>
              <div className="h-px bg-[#d1d5db] mb-2" />
              <p className="text-[11px] text-[#9ca3af]">{L.customerSignature}</p>
            </div>
            <div>
              <div className="h-px bg-[#d1d5db] mb-2" />
              <p className="text-[11px] text-[#9ca3af]">{L.date}</p>
            </div>
          </div>
        </div>
