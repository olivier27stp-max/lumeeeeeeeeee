...
/**
 * Tableau de rapport : grille CSS (même patron que la page Factures),
 * en-têtes triables, squelette de chargement, état vide, ligne de totaux
 * calculée côté serveur sur l'ensemble filtré (pas seulement la page).
 */
    ...
    <div className="border border-outline rounded-md overflow-x-auto bg-white dark:bg-[#0e0e11]">
      <div className="grid" style={{ gridTemplateColumns, minWidth: `${minWidth}px` }}>
        {columns.map((c) => {
          ...
          return (
            <div key={c.key} className={`py-3 px-4 border-b border-outline flex items-center text-[13px] font-medium text-text-primary ${align}`}>
              {c.sortable ? (
                <button
                  type="button"
                ...
                <span className="text-text-secondary">{c.label[lang]}</span>
              )}
            </div>
          );
        })}
          ...
          <React.Fragment key={`sk-${i}`}>
            {columns.map((c) => (
              <div key={c.key} className="py-3 px-4 border-b border-outline/30">
                <div className="h-4 w-24 max-w-full bg-surface-tertiary rounded animate-pulse" />
              </div>
            ))}
          </React.Fragment>
        ))}
        ...
        {rows.map((row, idx) => (
          <React.Fragment key={String(row.id ?? idx)}>
            {columns.map((c) => (
              <div
                key={c.key}
                className={`py-2.5 px-4 border-b border-outline/30 text-[13px] text-text-primary flex items-center min-w-0 ${isNumericColumn(c) ? 'justify-end tabular-nums' : ''} ${loading ? 'opacity-60' : ''}`}
                title={typeof row[c.key] === 'string' && (row[c.key] as string).length > 40 ? (row[c.key] as string) : undefined}
              >
                <span className="truncate">{formatCell(c, row[c.key], lang)}</span>
              </div>
            ))}
          </React.Fragment>
        ))}
        ...
        {hasTotals && rows.length > 0 && columns.map((c, i) => (
          <div
            key={`total-${c.key}`}
            className={`py-2.5 px-4 border-t border-outline bg-surface-secondary text-[13px] font-semibold text-text-primary flex items-center ${isNumericColumn(c) ? 'justify-end tabular-nums' : ''}`}
          >
            {i === 0 && !c.total ? (fr ? 'Totaux' : 'Totals') : (c.total === 'sum' ? formatTotal(c, totals?.[c.key], lang) : '')}
          </div>
        ))}
      </div>
      {totalsSkipped && (
