 ...
 * Source unique : le sélecteur (CompanyContext → orgApi.bureauActifSync), avec localStorage
 * en repli de démarrage — jamais « la première membership ».
 */
export function bureauActifPourEntete(): string | null {
  const publie = bureauActifSync();
 ...
 * affichées au lieu de 645. L'adhésion est vérifiée côté base : un en-tête forgé est ignoré.
 */
export function fetchAvecBureauActif(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
