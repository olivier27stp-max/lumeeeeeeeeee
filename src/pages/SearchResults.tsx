...
function PaginationControls({ page, totalPages, onChange }: { page: number; totalPages: number; onChange: (p: number) => void }) {
  ...
  return (
    <div className="mt-3 flex items-center justify-end gap-2">
      <button type="button" onClick={() => onChange(page - 1)} disabled={page <= 1} aria-label={t.common.previous} className="glass-button !px-2 !py-1 disabled:opacity-50">
        <ChevronLeft size={14} />
      </button>
      <p className="text-[11px] text-text-secondary">Page {page} / {totalPages}</p>
      <button type="button" onClick={() => onChange(page + 1)} disabled={page >= totalPages} aria-label={t.common.next} className="glass-button !px-2 !py-1 disabled:opacity-50">
