...
export const FONCTIONS: FonctionPage[] = [
  ...
  },
  {
    ...
    faq: [
      ...
      { q: { fr: 'C’est quoi un préréglage de soumission, et qu’est-ce qu’il contient ?', en: 'What is a quote preset, and what does it contain?' }, a: { fr: 'Un modèle de soumission prêt à envoyer, à ton nom : un titre, une description, une image de couverture, la liste de tes services avec prix et quantités, un texte d’introduction, tes conditions, et un dépôt (fixe ou en pourcentage) si tu en demandes un. À la création de ton compte, Lume te propose des services de départ selon ton métier (par exemple, en nettoyage résidentiel : ménage régulier, grand ménage, ménage de déménagement, lavage de tapis) ; ensuite, tes préréglages ne contiennent que ce que tu y as mis. Lumi ne connaît que tes préréglages à toi, pas un catalogue générique.', en: 'A ready-to-send quote template, in your name: a title, a description, a cover image, your list of services with prices and quantities, an intro text, your terms, and a deposit (fixed or percentage) if you ask for one. When your account is created, Lume suggests starter services for your trade (for residential cleaning: regular cleaning, deep cleaning, move-out cleaning, carpet washing); after that, your presets contain only what you put in them. Lumi only knows your own presets, not a generic catalogue.' } },
      { q: { fr: 'Jusqu’où je peux modifier un préréglage ?', en: 'How far can I edit a preset?' }, a: { fr: 'Entièrement : titre, description, images, services (ajouter, retirer, changer prix et quantités), texte d’intro, conditions, dépôt et sections personnalisées, dans Soumissions → Modèles et préréglages. Les soumissions déjà envoyées ne changent pas : un préréglage est un point de départ, chaque soumission garde sa propre copie. Un préréglage dont tu ne te sers plus se désactive plutôt que de se supprimer, pour garder l’historique.', en: 'Fully: title, description, images, services (add, remove, change prices and quantities), intro text, terms, deposit and custom sections, under Quotes → Templates and presets. Quotes already sent don’t change: a preset is a starting point, each quote keeps its own copy. A preset you no longer use is deactivated rather than deleted, to keep history.' } },
    ],
  ...
  },
  {
    ...
    faq: [
      ...
      { q: { fr: 'Mon paiement a échoué, je fais quoi ?', en: 'My payment failed, what do I do?' }, a: { fr: 'Deux cas. Ton abonnement Lume : va dans Paramètres → Forfait & facturation, le statut affiche « En retard » ; clique le bouton du portail de facturation (carte de crédit, factures, reçus) et mets ta carte à jour — le paiement est réessayé automatiquement et tu gardes l’accès pendant un délai de grâce de 7 jours. Le paiement d’un client sur une facture : il réessaie lui-même depuis le lien de sa facture avec une autre carte ; toi, tu n’as rien à faire, la facture reste « en attente » jusqu’au paiement.', en: 'Two cases. Your Lume subscription: go to Settings → Plan & billing, the status shows “Past due”; click the billing portal button (card, invoices, receipts) and update your card — payment is retried automatically and you keep access during a 7-day grace period. A client paying an invoice: they retry themselves from their invoice link with another card; nothing to do on your side, the invoice stays “pending” until paid.' } },
    ],
