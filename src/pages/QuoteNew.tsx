  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const detail = await createQuote({
        ...
        tax_rate: taxEnabled ? taxRate : 0,
        tax_rate_label: taxEnabled ? taxLabel : tq.noTax,
        tax_lines: taxEnabled ? resolvedTaxes.filter(tx => tx.is_active).map(tx => ({ name: tx.name, rate: tx.rate, amount_cents: 0, is_compound: tx.is_compound, tax_config_id: tx.id, registration_number: tx.registration_number || null })) : [],
