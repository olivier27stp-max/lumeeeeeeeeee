...
export default function Timesheets() {
  ...
  const approveEntries = async (ids: string[]) => { const orgId = await getCurrentOrgIdOrThrow(); for (const id of ids) { const { error } = await supabase.from('time_entries').update({ notes: '[APPROVED] ' + (entries.find(e => e.id === id)?.notes || '') }).eq('id', id).eq('org_id', orgId); if (error) { toast.error(error.message); loadData(); return; } } toast.success(fr ? `${ids.length} approuvé(s)` : `${ids.length} approved`); loadData(); setSelected(new Set()); };
